/*
    Program to test waiting for a signal to occur
...
void waitLoop(int seconds)
{
    ...
    for (i=seconds-1; i>=0; i--)
    {
        ...
        fflush(stdout);
        // Pause on multiples of 10
        if (i%10 == 0)
        {
            printf("Pausing to wait for a signal\n");
            pause();
        }
    }
    printf("\nTime out. Good bye\n");
...
void onInterrupt(int signal)
{
    printf("Catching the Ctrl-C signal: %d\n", signal);
}

void onStop(int signal)
{
    printf("Catching the Ctrl-Z signal: %d for the only time\n", signal);
}
